---
name: botones
description: Botones tocables en las respuestas del bot (opt-in) — el bot ofrece hasta 3 opciones que el cliente toca en vez de escribir (confirmar cita, elegir servicio, sí/no). Funcionan en WhatsApp, Instagram, Messenger y Telegram; en Twilio y ManyChat salen como lista numerada. Actívalo con "/botones", "quiero botones en mi bot", "que el bot mande opciones tocables", "botones de respuesta rápida", "quick replies", "apaga los botones".
---
...
Lee el `wrangler.toml` para ver qué canales tiene:

| Canal del bot | Cómo se ven |
|---|---|
| WhatsApp (Cloud API de Meta o YCloud) | Botones nativos de respuesta (máx 3) |
| Instagram / Messenger | Quick replies (chips tocables) |
| Telegram | Teclado de opciones de una sola vez |
| Twilio, chat web, ManyChat | Lista numerada en texto (fallback automático — nada se rompe) |
...
avanza sin repetir opciones. Si su canal es Twilio/web/ManyChat, muéstrale la
...
- No prometas botones nativos en canales de fallback (Twilio/ManyChat/web).
- Los botones no cambian frenos, tools ni escalación — es solo formato de salida.

## Nota técnica (para ti, no para el miembro)

El marcador `[[botones: A | B | C]]` lo parsea `extraeBotones()` en
`src/replies/sender.ts` y se limpia SIEMPRE del texto que ve el cliente, esté
`buttons_enabled` prendido o no — así un `system_prompt_override` también
puede usarlo sin que el switch lo sepa. `BUTTON_CHANNELS` (en
`src/channels/shared.ts`) decide render nativo vs. lista numerada por canal.
El tap regresa como texto normal del cliente en todos los canales — ninguno
necesita manejar un `callback_query` aparte.
