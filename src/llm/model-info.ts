/**
 * Bloque `model` del Centro de Mantenimiento (Forja Inbox — GET /api/maintenance).
 *
 * Porteo del paquete Forja+ v1.0.76, adaptado: el original lista 4 proveedores
 * (incluye "google"/Gemini); este fork solo enruta Anthropic/OpenAI/xAI (ver
 * `LlmProvider` en provider.ts) — Gemini no está conectado, así que no se
 * ofrece como opción real. `CURATED_MODELS` de este fork usa el campo
 * `label`, no `nombre` como el paquete original.
 *
 * Modo Agencia: el revendedor ve el MODELO REAL que usa cada bot (Claude/Grok/
 * GPT) + su costo, y puede cambiar de proveedor. Reusa `createModel` (resuelve
 * provider+modelo desde llm_provider/llm_model/BYO o el default del env) y
 * `src/pricing.ts` — sin inventar precios.
 *
 * `providerReady` es la fuente ÚNICA de "¿hay llave para este proveedor?": la
 * usan el picker (available_providers) del GET y el gate 409 del PATCH, para
 * que no se desincronicen.
 */
import type { Env } from "../env";
import { SETTING_KEYS } from "../db/settings";
import { createModel, envKeyFor, modelIdFor, CURATED_MODELS, type LlmProvider } from "./provider";
...
export const PROVIDERS: readonly LlmProvider[] = ["anthropic", "openai", "xai"];
...
/** Nombre bonito de un modelo: el `label` curado si lo conocemos, si no el id. */
function modelLabelOf(modelId: string): string {
  return CURATED_MODELS.find((m) => m.id === modelId)?.label ?? modelId;
}
 ...
 * mostramos el modelo inteligente (el techo real de capacidad/costo) y el
 * nivel "auto" viaja aparte en `tier`.
 */
