 ...
 * exactamente la misma maquinaria de aviso del handoff (Telegram DM /
 * WhatsApp template / email), cero duplicación.
 *
 * Garantías:
 ...
 *    (INSERT OR IGNORE antes de enviar).
 *  • Throttle global: máximo HOURLY_ALERT_CAP avisos por hora — una racha de
 *    análisis (p. ej. tras días sin correr) no metralla el teléfono del
 *    dueño. Se checa ANTES de reclamar: una conversación frenada por el
 *    throttle NO quema su claim y puede alertar en un re-análisis futuro.
...
export async function maybeAlertRisk(
  ...
  // Claim ANTES de enviar: si otro tick ya alertó esta conversación alguna
  // vez, INSERT OR IGNORE no escribe y salimos.
  ...
  // DASHBOARD_BASE_URL ya está siempre configurado en este bot — a
  // diferencia de Forja+, no hace falta resolución multi-fuente ni
  // heartbeat al control plane (que este proyecto evita a propósito).
  const who = conv.display_name || conv.channel_user_id;
  const base = (env.DASHBOARD_BASE_URL ?? "").replace(/\/+$/, "");
  const panelUrl = `${base}/admin/conversations?c=${encodeURIComponent(conversationId)}`;
  const summary =
    `⚠ ${who} (${conv.channel}): ${truncate(signal.summary, MAX_SUMMARY_CHARS)} ` +
    `Respóndele tú desde tu panel: ${panelUrl}`;

  // Si el aviso truena aquí, el claim se queda: mejor una alerta perdida que
  // un dueño recibiendo la misma dos veces.
