...
export function renderConexiones(env: Env): string {
  ...
  const cards = channels
    .map((ch) => {
      const badge = ch.ok
        ? `<span style="font-size:10px;letter-spacing:.14em;color:var(--ok);border:1px solid var(--ok);background:rgba(76,154,76,.08);padding:3px 10px;font-weight:700">● CONECTADO</span>`
      ...
      return `
        <div class="bg-panel border ${ch.ok ? "" : "border-line"}" style="padding:18px 20px;display:flex;flex-direction:column;gap:10px;${ch.ok ? "border-color:rgba(76,154,76,.45)" : ""}">
