/**
 * Centro de Mantenimiento (Forja Inbox móvil) — lo que el dueño/instalador
 ...
 *
 * Porteo del paquete Forja+ v1.0.76, adaptado a lo que este fork realmente
 * tiene: 3 proveedores de IA (no 4 — sin Gemini, ver llm/model-info.ts) y 6
 * superpoderes (no 9 — sin Botones/Galería/Bóveda/Cobros/Multi-idioma como
 * toggle separado, ver settings-mutations.ts).
 *
 * REGLA: SOLO settings que ya existen, reversibles al instante y validados
 * por el propio bot. NADA que pueda tumbarlo — ni deploy, ni secrets, ni
 * prompt override, ni KB, ni canales. La lista negra y la tabla de
 * validadores viven en settings-mutations.ts.
 *
 * Nada de esto aplica en caliente a una conversación en curso: el agente
 * relee settings.all() en el siguiente mensaje.
 */
...
async function buildMaintenance(c: Context<{ Bindings: Env }>) {
  ...
  return {
    ...
    // Modelo REAL + costo (Modo Agencia): proveedor/modelo efectivos, precio
    // por MTok y picker de proveedores con candados. month_usd repite el de
    // budget por conveniencia (mismo número).
    ...
    // Estado de configuración por superpoder — honestidad de toggles: los que
    // necesitan setup (Reseñas→link, Reenganche→plantilla) no se prenden "en
    // falso". Calculado EN VIVO.
    superpowers_meta: superpowersMeta(c.env, all),
    superpowers_pro: SUPERPOWERS_PRO,
    // Agenda de citas, SOLO lectura: distingue el "Horario de atención"
    // (informativo) de la agenda real. enabled = el nicho agenda citas por
    // diseño (ningún pack de nicho lo hace en este fork — ver niches/index.ts);
    // provider = "calcom" si el dueño puso la llave de Cal.com (independiente
    // del nicho en este fork, vía tools/scheduleAppointment.ts).
    scheduling: {
 ...
 * propio bot (settings-mutations.ts).
 */
const patchSchema = z
  .object({
    ...
    // Proveedor de IA. "" = Automático (default del env). Los 3 concretos se
    // validan contra "hay llave" (si no → 409 más abajo). Este fork no habla
    // con Gemini — ver llm/model-info.ts.
...
maintenanceApi.patch("/maintenance", async (c) => {
  ...
  // Cambio de proveedor de IA. NO pasa por la reja genérica — llm_provider es
  // NEVER_WRITABLE ahí a propósito; aquí se valida con la MISMA regla "hay
  // llave" del picker (providerReady) y se escribe directo, igual que el
  // panel. Se resuelve ANTES de escribir nada: un 409 no deja a medias.
  ...
  if (body.language !== undefined) {
    // "Espeja al cliente": en free el bot no lo corre (settings-loader no lo
    // gatea explícitamente, pero el criterio de este fork es igual que el del
    // panel — multi-idioma real es cortesía Pro), así que aquí se responde
    // igual que un toggle bloqueado.
  ...
  if (body.superpowers !== undefined) {
    ...
    // intenta PRENDER algo (apagar nunca se bloquea).
    const wantsOn = Object.values(body.superpowers).some((v) => v === true);
    const liveSettings = wantsOn ? await new SettingsRepo(new Db(c.env.DB)).all() : {};
    for (const [id, on] of Object.entries(body.superpowers)) {
      ...
      // Segunda disciplina: prender un superpoder que necesita setup y no lo
      // tiene → 409. Aunque la UI fallara, el backend no deja "activar" algo
      // roto. Apagar (`false`) SIEMPRE se permite.
      if (on) {
  ...
  // Segunda reja: el validador del propio bot, el mismo del panel. Se corre
  // sobre TODO antes de escribir NADA — un patch inválido no deja la mitad
  // aplicada.
  ...
  // idioma.ts no cachea por isolate en este fork (settings-loader relee D1
  // siempre) — bustIdiomaCache queda como no-op, se llama igual por si algún
  // día se agrega esa capa.
