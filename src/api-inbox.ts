...
import { agentStub } from "./agentStub"; // adaptado: nuestro módulo es camelCase
...
inboxApi.get("/conversations/:id/messages", async (c) => {
  ...
  // Conversación y página de mensajes en PARALELO (2026-08-29): mismo id, dos
  // round-trips a D1 que antes iban en fila — la app abre el hilo un viaje antes.
  const [conv, rows] = await Promise.all([
    new ConversationsRepo(db).getById(convId),
    db.all<{ id: string; role: string; content: string; created_at: number }>(
...
// app: persistir como owner + takeover + responder AL INSTANTE, y mandar por el
// adapter del canal en background (waitUntil). La falla común (ventana de 24 h
// de WhatsApp) rebota ANTES de persistir con su 409 legible; una falla rara del
// proveedor (token vencido, red) aterriza como nota interna EN el hilo con el
// motivo real — visible en app y panel. Nunca un "200 fantasma" silencioso:
// ese contrato (strict) sigue, solo cambió DÓNDE aterriza el error.
inboxApi.post("/conversations/:id/messages", async (c) => {
  ...
  const msgId = await msgs.append(conv.id, "owner", text);
  ...
  // Envío en BACKGROUND (2026-08-29, velocidad del app): el 200 sale ya
  // (~100ms percibidos) en vez de esperar los 300–1000ms del proveedor.
  // strict sigue: si el proveedor rechaza, el adapter LANZA con su motivo real
  ...
  const envio = (async () => {
    try {
      ...
      const resumen = text.length > 80 ? `${text.slice(0, 80)}…` : text;
      await msgs
        .append(conv.id, "note", `⚠️ Mensaje NO entregado: "${resumen}" — ${detail}. Reintenta o usa una plantilla.`)
        .catch(() => {});
    }
  })();
  try {
    c.executionCtx.waitUntil(envio);
  } catch {
    // tests / entornos sin executionCtx: la promesa corre suelta igual
  }

  return c.json(
    { ok: true, id: msgId, delivery: "sending", handoff_by: actor ? { name: actor.name, is_me: true } : null },
    200,
  );
});
...
// POST /api/admin-link — el botón "Panel admin" de Forja Inbox.
//
// ADAPTADO A PROPÓSITO: el paquete original hace SSO de un solo clic (token
// efímero que /admin/entrar/:token canjea por una sesión de cookie, vía el
// sistema de sesiones de /equipo — mintSession, panel_users, un middleware
// que reemplaza Basic Auth por esa cookie). Este fork NO tiene ese sistema:
// el panel se protege con Basic Auth y una sola contraseña compartida
// (DASHBOARD_PASSWORD, ver src/admin/auth.ts) — no hay usuarios individuales
// a los que darles una sesión propia.
//
// Portar el SSO real significaría construir /equipo entero (474 líneas:
// login con roles, invitaciones, recuperación de contraseña) solo para esta
// ruta — fuera de alcance de integrar Forja Inbox. La alternativa honesta:
// devolver la URL DIRECTA del panel. Al tocar "Panel admin" en la app se abre
// el panel real (con la marca BIRevX) y el navegador pide las credenciales de
// Basic Auth una vez — no es el SSO sin toque que ofrece el paquete original,
// pero es un enlace que de verdad funciona, no un token que no lleva a
// ningún lado.
inboxApi.post("/admin-link", async (c) => {
  const origin = await selfOriginFromRequest(c.env, c.req.url);
  return c.json({ ok: true, url: `${origin}/admin` }, 200);
});
...
// Adaptado: nombres REALES de las tools de agenda de este fork (ver
// src/tools/index.ts) — el paquete original usa "agendarCita"/"cancelarCita",
// que no existen acá. Incluye checkAvailability: sin ella el toggle "apagar
// agenda" de la app dejaría la consulta de horarios prendida a medias.
const AGENDAR_TOOL_NAMES = [
  "checkAvailability",
  "scheduleAppointment",
...
// Adaptado: el paquete original también reconoce Kapso y Zernio (env.KAPSO_API_KEY /
// env.ZERNIO_API_KEY) — este fork no tiene adapters para ninguno de los dos
// (sin src/channels/kapso.ts ni zernio.ts), así que se excluyen del check en
// vez de declarar env vars que no respaldan nada real.
function cloudWaConnected(env: Env): boolean {
  return !!((env.WHATSAPP_PHONE_NUMBER_ID && env.WHATSAPP_ACCESS_TOKEN) || env.YCLOUD_API_KEY);
}
...
inboxApi.post("/conversations/:id/template", async (c) => {
  ...
  // Adaptado: el paquete original resuelve `zctx` (Zernio, multi-plataforma)
  // para saber si el canal tiene ventana de 24h. Este fork no tiene adapter de
  // Zernio — se pasa `null` siempre, equivalente a "no es Zernio".
  if (!isWindowedChannel(conv.channel, null)) {
    return c.json(
      {
        ok: false,
  ...
  // Adaptado: de los proveedores "Cloud" del paquete original (whatsapp/kapso/
  // ycloud/zernio) este fork solo reconoce whatsapp/ycloud — y NINGUNO de los
  // dos tiene una función de envío de plantilla HSM implementada todavía (a
  // diferencia de Twilio, cuyo sendTwilioTemplate sí es real — lo usa
  // campaigns.ts en producción). Enviar una plantilla manual desde el hilo
  // solo funciona hoy si el canal es Twilio; para YCloud/Meta-directo se
  // rechaza ANTES de reservar cupo, honesto en vez de fingir que se mandó.
  const isCloudWa = conv.channel === "whatsapp" || conv.channel === "ycloud";
  if (isCloudWa) {
    return c.json(
      {
        ok: false,
        ...
        detail:
          "Mandar una plantilla manual desde el hilo todavía no está disponible para este canal — pídele a tu equipo que lo haga desde WhatsApp directo mientras tanto.",
      },
      501,
    );
  }
  ...
  await db.run(
    `INSERT INTO template_sends (campaign_key, conversation_id, kind, template_sid, sent_at)
     VALUES (?, ?, 'template', ?, ?)`,
    [campaignKey, conv.id, templateId, now],
  );

  // Texto renderizado: para que el agente tenga contexto cuando el cliente
  // responda ("SÍ"). Se busca ANTES de mandar.
  const { listContentTemplates } = await import("./campaigns");
  const tpl = (await listContentTemplates(c.env).catch(() => [])).find((t) => t.sid === templateId);
  const renderedText: string | null = renderTemplateBody(tpl?.body, variables);
  ...
  const msgId = await new MessagesRepo(db).append(
    conv.id,
    "owner",
    templateMarker(templateId) + (renderedText ?? `[plantilla ${templateId} enviada]`),
  );
  await convs.touchLastMessage(conv.id);
  await convs.setPausedUntil(conv.id, now + (await resolveTakeoverMs(c.env)));
  await convs.setTakenBy(conv.id, readActor(c));
