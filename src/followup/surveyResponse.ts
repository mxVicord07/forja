 ...
 * Este job busca encuestas sin calificar (survey_sends.score IS NULL) cuyo
 * cliente ya respondió después del envío, extrae el 1-5 de su mensaje y lo
 * guarda. Si la calificación es baja (≤2), avisa al dueño para que recupere
 * al cliente molesto — el "te avisa apenas queda molesto".
 *
 * Fuera del hot path (no toca el ingest): el cliente igual recibe la
 * respuesta normal del agente a su número; aquí solo registramos la métrica
 * + la alerta. Corre en el mismo cron que outreach.ts.
 */
  ...
  for (const row of rows) {
    ...
    if (score !== null && score <= LOW_SCORE_THRESHOLD) {
      try {
        ...
        const panelUrl = `${env.DASHBOARD_BASE_URL ?? ""}/admin/conversations?c=${encodeURIComponent(row.conversation_id)}`;
        await notifyOwner(env, {
          reason: "calificación baja",
          summary: `${who} te calificó ${score}/5. Vale la pena que lo recuperes tú.\n\nVer: ${panelUrl}`,
          ticketId: `survey:${row.conversation_id}`,
        });
        result.alerted++;
      } catch (e) {
