/**
 * Arma el reporte diseñado completo (skill /reportes): junta datos → IA
 * escribe insights → llena la plantilla (la custom del dueño si existe, si
 * no la default). Devuelve el HTML rico (página), el email-safe y el texto.
 * No manda nada — eso lo hace owner/dailyReport.ts (Telegram) o, si el skill
 * /reportes activa el canal, un envío de email futuro.
 */
...
export interface BuiltReport {
  ...
  title: string;
  subject: string;
  html: string; // reporte completo (página)
  emailHtml: string; // email-safe (teaser + botón) — sin canal de envío todavía en este fork
  text: string;
  model: ReportModel;
  empty: boolean;
}
...
/**
 * El reporte como DATOS: lo que se guarda en `report_last_json` y lo que
 * sirve GET /api/report/latest para Forja Inbox. La app lo pinta con sus
 * propias tarjetas, así que aquí no va nada de HTML — solo números, textos
...
/**
 * Cuerpo del reporte en markdown, para la app (Forja Inbox). Se arma SIEMPRE
 * desde el snapshot — el guardado o el recién construido — así hay un solo
 * renderer y no dos textos que se separen con el tiempo. Es el mismo
 * contenido que renderReportText, con marcas y sin el link a /admin/report:
 * en la app el reporte YA está abierto.
 */
