/**
 * Recolección de datos para el reporte diseñado (skill /reportes). Junta TODO
 * lo que el reporte muestra: conteos del día, tendencia (vs ayer y vs 7 días),
 * resolución, sentimiento, rating del bot, actividad por hora, temas,
 * preguntas sin responder, seguimientos del Cazador y reseñas. Y una muestra
 * de resúmenes: el material que la IA usa para escribir insights reales (no
 * inventa).
 *
 * Reemplaza el `collectDailyStats` simple que traía owner/dailyReport.ts —
 * mismo concepto, evolucionado (el paquete Forja+ v1.0.76 también lo trata
 * como una sola cosa: `collectDailyStats` vive aquí como wrapper delgado
 * "por compatibilidad con callers previos", no como una función paralela).
 */
...
/** Conteos de las últimas 24 h (compat con el `dailyReport.ts` anterior). */
