/**
 * Plantilla del reporte diseñado (skill /reportes) + render.
 *
 * Dos salidas del mismo modelo:
 *  • renderReportHtml → reporte COMPLETO (página): rico, con gráficas SVG.
 *    Es lo que se hostea en /admin/report. El dueño puede reemplazar la
 *    plantilla (report_template, la diseña Claude vía /reportes con su
 *    branding) mientras use los {{X}}.
 *  • renderReportEmail → email-safe compacto (resumen + KPIs + botón al
 *    completo). No se envía todavía en este fork (sin Resend wireado para
 *    esto) — queda listo para cuando el skill /reportes active ese canal.
 *
 * Las gráficas se generan aquí, coloreadas con el acento del negocio, y se
 * inyectan como placeholders — así una plantilla custom las reordena sin
 * recalcular.
 */
...
/** Texto plano — lo usa dailyReport.ts para el DM de Telegram. */
...
// Plantilla COMPLETA (página). CSS moderno + print para buena paginación
// (útil si el dueño la imprime o la exporta a PDF manualmente desde el navegador).
