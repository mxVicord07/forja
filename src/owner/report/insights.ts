/**
 * Insights del reporte diseñado (skill /reportes) — la parte inteligente.
 * Usa el MISMO modelo que el bot (workModel: el que el dueño configuró, con
 * el failover del agente detrás — ver llm/work-model.ts) para leer los datos
 * del día y escribir un análisis de negocio: resumen, hallazgos y acciones
 * para mañana. Grounded: solo razona sobre los datos que se le pasan, no
 * inventa. Si el modelo falla o devuelve basura, cae a un resumen mínimo con
 * los números.
 */
...
export async function generateReportInsights(env: Env, ctx: ReportContext): Promise<ReportInsights> {
  ...
  const material = [
    ...
    // Bug real del paquete Forja+ v1.0.76 corregido acá: ctx.topics es
    // {name,count}[], no string[] — un .join(", ") directo sobre el array de
    // objetos habría mandado "[object Object], [object Object]" al prompt.
    ctx.topics.length ? `\nTEMAS FRECUENTES HOY: ${ctx.topics.map((t) => t.name).join(", ")}` : "",
...
Devuelve SOLO un JSON válido, sin texto alrededor, con esta forma:
{
  ...
  try {
    const llm = await workModel(env, "smart", "reportes");
