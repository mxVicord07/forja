 ...
 * / parseScore). LLM + adapter + notifyOwner mockeados; D1 real via miniflare.
 */
...
import { captureSurveyResponses, parseScore } from "../../src/followup/surveyResponse";
...
async function seed(
  userId: string,
  opts: { userAt?: number; userMsgs?: number; endsWithUser?: boolean; channel?: string } = {},
): Promise<string> {
...
describe("captureSurveyResponses", () => {
  ...
  it("alerta al dueño cuando la calificación es baja (≤2)", async () => {
    ...
    const notice = notifyOwnerMock.mock.calls[0][1] as { reason: string; summary: string };
    expect(notice.summary).toContain("Beto");
    expect(notice.summary).toContain("1/5");
  });
